# -*- coding: utf-8 -*-
"""
Created on Tue Feb 11 16:19:41 2025
...
u_wind_file = "C:/Users/ferfo/OneDrive/Escritorio/anomuwnd-2012-850.nc"
v_wind_file = "C:/Users/ferfo/OneDrive/Escritorio/anomvwnd-2012-850.nc"
...
if 'pressure_level' in u_anomalies.dims:
    u_anomalies = u_anomalies.sel(pressure_level=850.0)
    v_anomalies = v_anomalies.sel(pressure_level=850.0)
elif 'level' in u_anomalies.dims:  # Si la dimensión se llama 'level'
    u_anomalies = u_anomalies.sel(level=850.0)
    v_anomalies = v_anomalies.sel(level=850.0)
...
levels = np.arange(0, 10.1, 1)  # Niveles de magnitud de viento
...
quiver = ax.quiver(
    ...
    transform=ccrs.PlateCarree(),
    scale=200,  # Ajusta la escala para la longitud de las flechas
    color='black',
    width=0.002
)
...
quiver_key = ax.quiverkey(
    quiver, 
    X=0.95, Y=-0.05, U=7,  # Centrar y bajar la flecha debajo del gráfico
...
# Guardar la figura
plt.savefig("C:/Users/ferfo/OneDrive/Escritorio/wind-850-2.png", dpi=300, bbox_inches='tight')
